...
* When the program finishes, you should have a binary file by the same name as your input file. If this didn't work, there is a chance your llvm executables are in a different place. Edit line 9 of the file `/usr/local/DeafShark/compileandlink.sh` to point to the location of `llc` on your machine. If this still doens't work, then it is most likely because your using parts of the language that have not been developed yet.
